...
<div class="main-content">
    ...
    <table class="table table-striped table-hover">
        ...
        <tbody>
            ...
            try {
                // Prepare and execute the query to fetch all users, using firstname instead of name
                $stmt = $conn->prepare("SELECT usersId, firstname, email, job_title, username, created_at FROM users ORDER BY created_at DESC");
                $stmt->execute();
                ...
                if ($users && count($users) > 0) {
                    foreach ($users as $user) {
                        ...
                        echo "<td>" . htmlspecialchars($user['firstname']) . "</td>"; // Changed from 'name' to 'firstname'
